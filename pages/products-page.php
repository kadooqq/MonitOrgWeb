    ...
    <table class="table" id="myTable">
        ...
        <?php foreach ($products as $product) :?>
            ...
            <tr>
                ...
                <td class="align-middle"><?= empty($product->get_document()) ? '?' : $product->get_steel_mark()?></td>
                ...
                <td class="align-middle"><?= ($product->get_diameter_min() == 0 ? '?' : $product->get_diameter_min()).'-'.($product->get_diameter_max() == 0 ? '?' : $product->get_diameter_min())?></td>
                <td class="align-middle"><?= ($product->get_width() == 0 ? '?' : $product->get_width()).'x'.($product->get_height() == 0 ? '?' : $product->get_height())?></td>
                <td class="align-middle"><?= ($product->get_thickness_min() == 0 ? '?' : $product->get_diameter_min()).'-'.($product->get_thickness_max() ? '?' : $product->get_thickness_max())?></td>
