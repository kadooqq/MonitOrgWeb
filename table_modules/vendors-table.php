...
class VendorsTable {
    ...
    private static function _get_vendor_from_row(mixed $row) : Vendor {
        $vendor = new Vendor();
    ...
    {
        $query =  DataDBconnection::prepare("select * from ".static::$table_name.";");
        $query->execute();
        $result = $query->fetchAll();
        $array_vendors = array();

        foreach ($result as $row) {
            $array_vendors[] = self::_get_vendor_from_row($row);
        }
    ...
    public static function select_by_id(int $id) : Vendor | null {
        $query = DataDBConnection::prepare("SELECT * FROM vendors WHERE vendor_id= :id;");
        ...
        if (!count($result))
            return null;
        return self::_get_vendor_from_row($result[0]);
    }

    public static function create(Vendor $vendor) {
        $query = DataDBConnection::prepare("INSERT INTO vendors (vendor_name, vendor_shortname, vendor_region) VALUES (:name, :shortname, :region);");
    ...
    public static function update(Vendor $vendor) {
        $query = DataDBConnection::prepare("UPDATE vendors SET vendor_name = :name, vendor_shortname = :shortname, vendor_region = :region WHERE vendor_id = :id;");
    ...
    public static function remove_by_id($id) {
        $query = DataDBConnection::prepare("DELETE FROM vendors WHERE vendor_id = :id");
        $query->bindValue(":id", $id);
