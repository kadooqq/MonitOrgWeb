...
class ProductsTable {
    ...
    private static function _get_product_from_row(mixed $row) : Product {
        $product = new Product();
    ...
    {
        $query = DataDBConnection::prepare("SELECT * FROM vendors JOIN products ON vendors.vendor_id = products.vendor_id");
        $query->execute();
        ...
        foreach ($result as $row) {
            $array_products[] = self::_get_product_from_row($row);
        }
    ...
    public static function select_by_id(int $id) : Product | null {
        $query = DataDBConnection::prepare("SELECT FROM vendors join products on vendors.vendor_id = products.vendor_id WHERE product_id= :id;");
        ...
        if (!count($result))
            return null;
        return self::_get_product_from_row($result[0]);
    }

    public static function create(Product $product) {
        $query = DataDBConnection::prepare("INSERT INTO ".static::$table_name.
            "INSERT INTO products (vendor_id,product_name,product_seamType,product_subType,product_document,product_diameterMin,product_diameterMax,product_width,product_height,product_thicknessMin,product_thicknessMax,product_steelMark,product_price,product_status)"
        ...
        $query->bindValue(":product_status", $product->get_status());
    ...
    public static function update(Product $product) {
        $query = DataDBConnection::prepare("UPDATE products SET vendor_id = :vendor_id , product_name = :name, product_seamType = :seam_type, product_subType = :sub_type, product_document = :document, product_diameterMin = :diameter_min, product_diameterMax = :diameter_max, product_width = :width, product_height = :height, product_thicknessMin = :thickness_min, product_thicknessMax = :thickness_max, product_steelMark = :steel_mark, product_price = :price, product_status = :status".
            " WHERE product_id = :id;");
    ...
    public static function remove_by_id($id) {
        $query = DataDBConnection::prepare("DELETE FROM ".static::$table_name." WHERE product_id = :id");
        $query->bindValue(":id", $id);
